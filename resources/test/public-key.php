<?php

/**
 * @file
 * Provides the official NovaPay sandbox public key fixture.
 */

declare(strict_types=1);

/**
 * Official NovaPay sandbox public key.
 *
 * This public test fixture is documented in docs/novapay_kb_general.md and
 * must never be used to verify production responses or postbacks.
 */
return <<<'PEM'
-----BEGIN PUBLIC KEY-----

/wIDAQAB
-----END PUBLIC KEY-----
PEM;
